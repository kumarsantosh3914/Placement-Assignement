...
function handleProductClick(event) {
  if (event.target.classList.contains("add-to-cart")) {
    ...
    const product = getProductById(productId);
    if (product) {
      saveProductToLocalStorage(product); // Save the product data to localStorage
      window.location.href = "product.html"; // Redirect to the single product page
    }
  }
}
